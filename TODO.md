...
- The custom `.archon/commands/dev-update-kb.md` instructs an agent to read the highest `INC-YYYY-NNN` from `INCIDENT-LOG.md`, render an entry by hand and append it; `dev-security.md:81-82` reinforces it as "the concurrent-write fix". SYSTEM.md Rule 8 now bans this outright — the log is a generated view and hand-editing it is the exact pattern that silently lost three findings in two days. `devsys incidents file` replaces it. Confirmed not to exist in any other project's `.archon` folder. Moot if the dev-pipeline commands are removed.
