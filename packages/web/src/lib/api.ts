...
export async function listKanbanTasks(options?: {
  project?: string;
  status?: KanbanTaskStatus;
  limit?: number;
  offset?: number;
  ...
  if (options?.offset) params.set('offset', String(options.offset));
  const qs = params.toString();
  const result = await fetchJSON<{ tasks: KanbanTask[] }>(`/api/tasks${qs ? `?${qs}` : ''}`);
  return result.tasks;
}
...
export async function createKanbanTask(input: {
  ...
  return fetchJSON<KanbanTask>('/api/tasks', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  });
...
export async function updateKanbanTask(
  ...
  return fetchJSON<KanbanTask>(`/api/tasks/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(updates),
  });
...
export async function deleteKanbanTask(id: string): Promise<{ success: boolean }> {
